...
### Fixed
- **"Review all edits at end of turn" produced no review at all with Opus 5.** For the feature to work, every file edit has to pass through Code Astrogator's permission hook — that is the only moment at which the file can be snapshotted *before* Claude writes to it, which is what the end-of-turn diff is built from. To get the edits there, Code Astrogator asked the CLI for its "ask before everything" mode by simply not specifying a mode at all. With the current Claude Code CLI that no longer means "ask": it means "auto", and in auto mode the model itself decides whether to ask. Measured in one project with identical settings and only the model swapped: Haiku asked for a one-line edit, Opus 5 applied the very same edit without asking at all. No snapshot was taken, so nothing could be reviewed — and because auto-accepted edits look identical either way, the chat gave no hint that anything had gone wrong. The mode is now requested explicitly instead of being left to the CLI's default. Since the name of that mode changed between CLI versions, Code Astrogator reads the list of modes the installed CLI actually supports (once per CLI, in the background, no console window) and picks the right one, falling back to the previous behaviour on older CLIs. The same problem quietly affected **"Ask before edits"**, which had stopped asking before applying file edits — that works again too.
- **"Review all edits at end of turn" could also be switched on and still do nothing.** The toggle in the Model·Mode popover is drawn from the saved settings, while the running turn was driven by a separate in-memory copy — and the two were only ever brought into line when the chat window was created or when you clicked the toggle itself. If they drifted apart, the popover kept showing the feature as on while the turn ran with it off: the CLI applied the edits on its own, no pre-edit snapshot was taken, and no list of changed files appeared at the end of the turn. The edits still showed the same green "auto-approved" card with a diff, so there was nothing to see that anything was wrong. The saved settings are now the single source of truth: they are mirrored onto the session when the window opens, whenever a setting changes, and once more before every prompt is sent — and any host-side change is pushed straight back into the popover, so a control can no longer show a state the extension is not actually running with. Settings are also loaded earlier during startup, before the chat window can be created.
- **The end-of-turn review no longer fails quietly.** If a turn ends with nothing to review although the option is on, the chat now says why instead of showing nothing: either the turn was not armed (with the "switch it off and on again" remedy), or individual files had to be dropped because their pre-edit state could not be read — those are deliberately never reverted, since reverting against an unreliable snapshot could destroy your work.

### Changed
- **Re-verified the permission hook against Claude Code CLI 2.1.263** (the notes were still on 2.1.178). The MCP protocol, the streamed tool result and the guarantee that the CLI waits for our reply before writing a file are all unchanged. The permission modes were renamed, though: the old default/ask mode is now called `manual`, and leaving the flag off no longer means "ask" — see the fix above. One thing to keep in mind: if you set `permissions.defaultMode` in your own `~/.claude/settings.json` to a mode that applies edits without asking (`acceptEdits`, `dontAsk`, `bypassPermissions`), that still takes edits off the hook, and the end-of-turn review then has nothing to build a diff from — which it now reports instead of staying silent.
